...
function getCells(size: number): (0 | 1)[] {
  const arr: (0 | 1)[] = [];

  for (let i = 0; i < size; i++) {
    arr.push(Math.round(Math.random()) as 0 | 1);
    // arr.push(0);
  }
...
document.addEventListener('keydown', function ({ code }: KeyboardEvent) {
  ...
  if (gameRunning) window.cancelAnimationFrame(animationFrameId);
